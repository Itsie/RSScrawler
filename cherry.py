...
<html lang="de">
  ...
  <div class="container">
    ...
    <form id="rsscrawler" enctype="multipart/form-data" method="post" action="speichern" onsubmit="setTimeout(function () { window.location.reload(); }, 10)">
          ...
          Suchfunktion statt Feed nutzen:<div hinweis="Wenn aktiviert, wird die Suchfunktion des Movie-Blog genutzt, da der Feed nur wenige Stunden abbildet (True oder False)."><input type="text" name="historical" value="''' + historical +'''"></div>
          3D-Releases suchen:<div hinweis="Wenn aktiviert, sucht das Script zusätzlich auch nach 3D Releases (in 1080p), unabhängig von der oben gesetzten Auflösung (True oder False)."><input type="text" name="crawl3d" value="''' + crawl3d +'''"></div>
          ...
          <br /></div>
          Staffeln suchen:<div hinweis="Wenn aktiviert, werden komplette Staffeln entsprechend der Staffel-Liste auf Movie-Blog gesucht. (True oder False)"><input type="text" name="crawlseasons" value="''' + crawlseasons +'''" ></div>
          ...
          <div hinweis="Dieser Bereich ist für die Suche auf Serienjunkies.org zuständig."><h3>SerienJunkies</h3></div>
          ...
          Auch per RegEx-Funktion suchen:<div hinweis="Wenn aktiviert, werden Serien aus der Serien (RegEx)-Liste nach den entsprechenden Regeln gesucht (True oder False)."><input type="text" name="regex" value="''' + regex +'''"></div>
