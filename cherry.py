...
<html lang="de">
  <head>
    ...
    <style>
      @import url(https://fonts.googleapis.com/css?family=Roboto:400,300,600,400italic);.copyright,[title]:before,div,h1,h2,h3,input{text-align:center}*{margin:0;padding:0;box-sizing:border-box;-webkit-box-sizing:border-box;-moz-box-sizing:border-box;-webkit-font-smoothing:antialiased;-moz-font-smoothing:antialiased;-o-font-smoothing:antialiased;font-smoothing:antialiased;text-rendering:optimizeLegibility}body{font-family:Roboto,Helvetica,Arial,sans-serif;font-weight:100;font-size:14px;line-height:30px;color:#000;background:#d3d3d3}.container{max-width:800px;width:100%;margin:0 auto;position:relative}[title]:after,[title]:before{position:absolute;bottom:150%;left:50%}#rsscrawler button[type=submit],#rsscrawler iframe,#rsscrawler input[type=text],#rsscrawler textarea{font:400 12px/16px Roboto,Helvetica,Arial,sans-serif}#rsscrawler{background:#F9F9F9;padding:25px;margin:50px 0;box-shadow:0 0 20px 0 rgba(0,0,0,.2),0 5px 5px 0 rgba(0,0,0,.24)}#rsscrawler h1{display:block;font-size:30px;font-weight:300;margin-bottom:10px}#rsscrawler h4{margin:5px 0 15px;display:block;font-size:13px;font-weight:400}fieldset{border:none!important;margin:0 0 10px;min-width:100%;padding:0;width:100%}#rsscrawler iframe,#rsscrawler input[type=text],#rsscrawler textarea{width:100%;border:1px solid #ccc;background:#FFF;margin:0 0 5px;padding:10px}#rsscrawler iframe,#rsscrawler input[type=text]:hover,#rsscrawler textarea:hover{-webkit-transition:border-color .3s ease-in-out;-moz-transition:border-color .3s ease-in-out;transition:border-color .3s ease-in-out;border:1px solid #aaa}#rsscrawler textarea{height:100px;max-width:100%;resize:none}#rsscrawler button[type=submit]{cursor:pointer;width:100%;border:none;background:#333;color:#FFF;margin:0 0 5px;padding:10px;font-size:15px}#rsscrawler button[type=submit]:hover{background:#43A047;-webkit-transition:background .3s ease-in-out;-moz-transition:background .3s ease-in-out;transition:background-color .3s ease-in-out}#rsscrawler button[type=submit]:active{box-shadow:inset 0 1px 3px rgba(0,0,0,.5)}#rsscrawler input:focus,#rsscrawler textarea:focus{outline:0;border:1px solid #aaa}::-webkit-input-placeholder{color:#888}:-moz-placeholder{color:#888}::-moz-placeholder{color:#888}:-ms-input-placeholder{color:#888}[title]{position:relative;z-index:2;cursor:pointer}[title]:after,[title]:before{visibility:hidden;-ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)";filter:progid: DXImageTransform.Microsoft.Alpha(Opacity=0);opacity:0;pointer-events:none}[title]:before{margin-bottom:5px;margin-left:-400px;padding:9px;width:782px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:0;background-color:#000;background-color:hsla(0,0%,20%,.9);color:#fff;content:attr(title);font-size:14px;line-height:1.2}[title]:after{margin-left:-5px;width:0;border-top:5px solid #000;border-top:5px solid hsla(0,0%,20%,.9);border-right:5px solid transparent;border-left:5px solid transparent;content:" ";font-size:0;line-height:0}[title]:hover:after,[title]:hover:before{visibility:visible;-ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=100)";filter:progid: DXImageTransform.Microsoft.Alpha(Opacity=100);opacity:1}
    </style>
  </head>
  <body>
  <div class="container">
    ...
    <form id="rsscrawler" enctype="multipart/form-data" method="post" action="logleeren?wert=1" target="speichernFrame" onsubmit="setTimeout(function () { window.location.reload(); }, 10)">
          <h1>Log</h1>
            <iframe sandbox="allow-forms allow-same-origin allow-pointer-lock allow-scripts allow-popups allow-modals" data-title="RSScrawler" src="./log" width="800" height="240">
    ...
    <form id="rsscrawler" enctype="multipart/form-data" method="post" action="speichern" onsubmit="setTimeout(function () { window.location.reload(); }, 10)">
          ...
          Prefix:<div title="Hier den Prefix des Webservers für Einstellungen und Log wählen (nützlich für Reverse-Proxies)."><input type="text" name="prefix" value="''' + prefix +'''"></div>
          Suchintervall:<div title="Das Suchintervall in Minuten sollte nicht zu niedrig angesetzt werden um keinen Ban zu riskieren. Minimum sind 3 Minuten!"><input type="text" name="interval" value="''' + interval +'''"></div>
          Pushbullet API-Schlüssel:<div title="Um über hinzugefügte Releases informiert zu werden, hier den Pushbullet API-Key eintragen."><input type="text" name="pushbulletapi" value="''' + pushbulletapi +'''"></div>
          Hoster:<div title="Hier den gewünschten Hoster eintragen (Uploaded oder Share-Online). Möglich sind auch beide (durch Kommata getrennt)."><input type="text" name="hoster" value="''' + hoster +'''"></div>
          <div title="Dieser Bereich ist für die Suche auf Movie-Blog.org zuständig."><h3>Movie-Blog</h3></div>
          Auflösung:<div title="Die Release-Auflösung, nach der gesucht wird (1080p, 720p oder 480p)."><input type="text" name="mbquality" value="''' + mbquality +'''"></div>
          ...
          <div title="Dieser Bereich ist für die Suche auf Serienjunkies.org zuständig."><h3>SerienJunkies</h3></div>
          <p>Auflösung:<div title="Die Release-Auflösung, nach der gesucht wird (1080p, 720p oder 480p)."><input type="text" name="sjquality" value="''' + sjquality +'''"></div>
          Filterliste:<div title="Releases mit diesen Begriffen werden nicht hinzugefügt (durch Kommata getrennt)."><input type="text" name="rejectlist" value="''' + rejectlist +'''"></div>
          Auch per RegEx-Funktion suchen:<div title="Wenn aktiviert, werden Serien aus der Serien (RegEx)-Liste nach den entsprechenden Regeln gesucht (True oder False)."><input type="text" name="regex" value="''' + regex +'''"></div>
          <button type="submit">Speichern</button>
    </form>
    <form id="rsscrawler" action="https://www.9kw.eu/register_87296.html" target="_blank" onsubmit="setTimeout(function () { window.location.reload(); }, 10)">
          <h1>Captchas</h1>
          <button type="submit">Captchas automatisch lösen lassen</button>
    </form>
    <form id="rsscrawler" enctype="multipart/form-data" method="post" action="listenspeichern" onsubmit="setTimeout(function () { window.location.reload(); }, 10)">
          <h1>Suchlisten</h1>
          <div title="Pro Zeile ein Filmtitel."><h3>Filme</h3></div>
          <textarea name="mbfilme">''' + self.getListe('MB_Filme') + '''</textarea>
          <div title="Pro Zeile ein Serientitel für ganze Staffeln."><h3>Staffeln</h3></div>
    ...
    <form id="rsscrawler" enctype="multipart/form-data" method="post" action="neustart?wert=1" target="speichernFrame" onsubmit="setTimeout(function () { window.location.reload(); }, 10)">
